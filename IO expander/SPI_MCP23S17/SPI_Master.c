...
{
	DDRB = 0b10110001;					// make the master pins as output
	PORTB |= (1<<0);
...
{
	PORTB &= ~(1<<4);					// sets slave select low (SS is active-low)
}

void SPI_Deselect(void)
{
	PORTB |= (1<<4);					// sets slave select high (SS is active-low)
}
